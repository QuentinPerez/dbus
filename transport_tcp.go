...
func (o *tcpReader) Read(b []byte) (n int, err error) {
...
func newTCPTransport(addr string) (transport, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("newTCPTransport.ResolveTCPAddr: %v", err)
	}
	t.TCPConn, err = net.DialTCP("tcp", nil, tcpAddr)
	if err != nil {
		return nil, fmt.Errorf("newTCPTransport.DialTCP: %v", err)
	}
	return t, nil
